...
```{r}
ndocs <- length(reddit$comments)
minDocFreq <- ndocs * 0.01
maxDocFreq <- ndocs * 0.9

c <- Corpus(VectorSource(reddit$comments))
dtm <- DocumentTermMatrix(c, control = list(bounds = list(global = c(minDocFreq, maxDocFreq))))

terms <- tidy(dtm)
```

Make a document term matrix from the comments in the dataset.  The parameters minDocFreq and maxDocFreq are use to filter out overly common words, so we can identify keywords we are looking for.

```{r}
model <- udpipe_download_model(language = "english")
udmodel_english <- udpipe_load_model(model)
s <- udpipe_annotate(udmodel_english, terms$term)
x <- data.frame(s)
```
...
x <- data.frame(s)
x %<>%
  filter(!grepl("page|resume|role|analyst|science|thing|previous|luck|scientist|book|reply|job|kind|media|source|mate|i|clearance|citzenship|bro|one|company|old|offers|jobs|entry|level|sorry", lemma))
